...
class PageTable:
	...
	def assemble_nearby_pages(self, obj, lst):
		pgt_ptr = obj.pagetable
		x_mod = obj.tile_x % PAGETABLE_N
		y_mod = obj.tile_y % PAGETABLE_N
		lst.append(pgt_ptr.get_page_lstexc(obj.page_row, obj.page_col))
		if x_mod >= PAGETABLE_N - ONETHIRD_PAGETABLE_N:
			if y_mod >= PAGETABLE_N - ONETHIRD_PAGETABLE_N:
				lst.append(pgt_ptr.get_page_lstexc(obj.page_row + 1, obj.page_col + 1))
				lst.append(pgt_ptr.get_page_lstexc(obj.page_row + 1, obj.page_col))
				lst.append(pgt_ptr.get_page_lstexc(obj.page_row, obj.page_col + 1))
